...
def create_database(directory, show=True):
    ...
    for filename in fnmatch.filter(os.listdir(directory),'*.jpg'):
        ...
        result = gray[rects[1]:rects[3], rects[0]:rects[2]]
...
def sort_eigenvals(eigenvalues, eigenvectors):
    """
    Sort eigenvalues and corresponding vectors in decreasing order

    @param eigenvalues: np array of eigenvalues
    @param eigenvectors: np array of eigenvectors (dimension x number_of_vectors)
    """

    sortedvals = sorted(eigenvalues, reverse=True)
    sortedvecs = np.zeros_like(eigenvectors)
    eigenvalues = eigenvalues.tolist()

    for i in range(len(sortedvals)):
        ind = eigenvalues.index(sortedvals[i])
        sortedvecs[:, i] = eigenvectors[:, ind]

    return np.array(sortedvals), sortedvecs
...
def pca(X, nb_components=0):
    ...
    C = B.dot(B.transpose()).dot(1./(n-1))
    vals, vectors = np.linalg.eig(C)
    vals, vectors = sort_eigenvals(vals, vectors)
    vals = vals[:nb_components]
    vectors = vectors[:nb_components, :]

    vectors = B.transpose().dot(vectors)
    vectors = norm_vectors(vectors)

    return vals, vectors, mu
